"""Ticket business logic shared between the web API and the Slack bot.

Keeping this here (rather than in the route/handler) means status sync,
validation, and creation behave identically no matter where the request
originates, and it stays easy to unit-test without a live Slack or HTTP layer.
"""
from app.models import STATUSES, Comment, Ticket, db
...
def list_tickets(view="all", user=None):
    ...
    if view in ("mine", "mine_open"):
        ...
        if view == "mine_open":
            from app.models import STATUS_CLOSED
